...
static void EncodeBrute(u8* out, s16* inBuffer, s32* origState, s32*** coefTable, s32 order, s32 npredictors, u32 framesize) {
	s16 ix[16];
	s32 prediction[16];
	s32 inVector[16];
	s32 optimalp = 0;
...
static void ReadBook(AudioSample* sampleInfo, s32**** table, s32* destOrder, s32* destNPred) {
	s32** tableEntry;
	
	s32 order = *destOrder = sampleInfo->vadBook.cast.u16[0];
	s32 nPred = *destNPred = sampleInfo->vadBook.cast.u16[1];
	
	*table = malloc(sizeof(void*) * nPred);
	for (s32 i = 0; i < nPred; i++) {
		(*table)[i] = malloc(sizeof(void*) * 8);
		for (s32 j = 0; j < 8; j++) {
...
static void EncodeFrame(Memfile* mem, s16* buffer, s32* state, s32*** coefTable, s32 order, s32 npredictors, u32 framesize, u8 precision) {
...
static void DecodeFrame(u8* frame, s32* decompressed, s32* state, s32 order, u32 npred, s32*** coefTable, u32 framesize) {
	s32 ix[16];
	...
	for (s32 j = 0; j < 2; j++) {
		...
		for (s32 i = 0; i < 8; i++) {
			...
			if (optimalp >= npred)
				optimalp = clamp_max(optimalp, npred - 1);
...
void Audio_VadpcmTableDesign(AudioSample* sampleInfo) {
	...
	#define FREE_P(P) if (P) delete(P)
	
	u32 refineIteration = sint(gTableDesignIteration);
	u32 frameSize = sint(gTableDesignFrameSize);
	u32 bits = sint(gTableDesignBits);
	u32 order = sint(gTableDesignOrder);
	f64 threshold = sfloat(gTableDesignThreshold);
	...
	if (numOverflows) {
		warn("\aTableDesign seems to have " PRNT_REDD "overflown" PRNT_RSET " [%d] times!", numOverflows);
	}
...
void Audio_VadpcmEncode(AudioSample* sampleInfo) {
	...
	if (sys_stat(x_fmt("%s.normalize", x_path(sampleInfo->input))))
		Audio_Normalize(sampleInfo);
	
	if (sampleInfo->vadBook.data == NULL)
		Audio_VadpcmTableDesign(sampleInfo);
	...
	ReadBook(sampleInfo, &table, &order, &nPred);
	...
	if (sampleInfo->instrument.loop.count) {
		...
		while (pos <= loopStart) {
			...
			EncodeFrame(&memEnc, buffer, state, table, order, nPred, 16, prec);
			pos += 16;
			nBytes += prec;
		}
	...
	while (pos < nFrames) {
		...
		if (nFrames - pos >= nSam) {
			memcpy(buffer, &sampleInfo->audio.s16[pos], sizeof(s16) * 16);
			EncodeFrame(&memEnc, buffer, state, table, order, nPred, nSam, prec);
			pos += nSam;
			nBytes += prec;
		} else {
			warn("VadpcmEnc: Missed a frame!");
			break;
		}
	}
	...
	osLog("Old Memfile Size [0x%X]", sampleInfo->size);
	...
	osLog("New Memfile Size [0x%X]", sampleInfo->size);
}

extern bool gSdkAudioError;

void Audio_VadpcmDecode(AudioSample* sampleInfo) {
	Memfile memDec = Memfile_New();
	s32 order;
	s32 npredictors;
	s32*** coefTable = NULL;
	u32 nSamples = 0;
	...
	ReadBook(sampleInfo, &coefTable, &order, &npredictors);
	...
	while (true) {
		...
		// Decode for real
		DecodeFrame(input, decompressed, state, order, npredictors, coefTable, framesize);
		...
		EncodeBrute(encoded, guess, lastState, coefTable, order, npredictors, framesize);
		...
		if (memcmp(input, encoded, framesize) != 0) {
			...
			if (bruteforce(guess32, input, decoded, decompressed, lastState, coefTable, order, npredictors, framesize)) {
				for (int i = 0; i < 16; i++) {
					...
					//     warn("-0x8000 <= guess32[i] && guess32[i] <= 0x7fff");
					guess[i] = clamp(guess32[i], -__INT16_MAX__, __INT16_MAX__);
				}
				EncodeBrute(encoded, guess, lastState, coefTable, order, npredictors, framesize);
				// if (memcmp(input, encoded, framesize) != 0)
//     warn("Could not bruteforce sample %d", nSamples);
			}
...
void Audio_VadpcmBookLoad(AudioSample* sampleInfo, const char* file) {
	...
	if (vadBook->data) {
		osLog("VadBook already exists");
	}
	
	Memfile_LoadBin(&temp, file);
